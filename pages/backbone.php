...
$volunteersStats = $this->database->query(
	'SELECT tshirt_size, tshirt_cut, food_preferences, lang, COUNT(*) as count FROM users GROUP BY tshirt_size, tshirt_cut, food_preferences, lang'
);
...
foreach ($volunteersStats as $volunteersStat) {
	$stats['tshirt_cut'][$volunteersStat->tshirt_cut] += $volunteersStat->count;
	$stats['tshirt_size'][$volunteersStat->tshirt_size] += $volunteersStat->count;
	...
	<div class="pane">
		...
		<table>
			...
			<tbody>
				<tr>
					<td>
						<?php foreach ($stats['tshirt_size'] as $size => $count): ?>
							<?php echo htmlspecialchars(ucfirst($size)) . ': ' . htmlspecialchars($count) . '<br>'; ?>
						<?php endforeach; ?>
					</td>
					<td>
						<?php foreach ($stats['tshirt_cut'] as $cut => $count): ?>
							<?php echo htmlspecialchars(ucfirst($cut)) . ': ' . htmlspecialchars($count) . '<br>'; ?>
						<?php endforeach; ?>
					</td>
					<td>
